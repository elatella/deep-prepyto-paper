...
Two datasets of different origins were used as input and test subjects for the automatic segmentation pipeline, respectively.
They consisted of rat synaptosomes primary neuron cultures derived from mice.
...
Binarization of probability masks

Post Processing on Biniraized map (adaptive threshold, radial profile, outlier removal)
...
In short, the segmented region is divided into 1 voxel thick layers parallel to the active zone for distance calculations.
A hierarchical connectivity segmentation detects densities interconnecting boundaries.
The boundaries were synaptic vesicles and the active zone PM. Detected intravesicular segments are termed connectors and segments connecting vesicles to the active zone PM are called tethers (Figure `\_add figure number*`{.green}).
...
2. 9 synaptosome datasets were used for the test.
3. 12 Neuron datasets were used for assessing transfer learning potential.
...
### Stitching Patches Back Together (building probability map?)
...
To mitigate tiling effects during testing, the network input can be expanded to a larger volume like 64 voxels.
...
We used a U-Net with two downsampling stages and two convolutional layers per stage, with a kernel size of 3, and a ReLU activation function based on the open-source CARE framework (Figure {@fig:unet}) [@doi:10.1038/s41592-018-0216-7]. 
Datasets were prepared by splitting the 3D tomographic volume of synaptosomes into 32^3^-voxel subvolumes and keeping only subvolumes occupied by a sufficient amount (> 1000 voxels) of binarized vesicle labels.
...
The initial spherical conversion was done by setting the center of the sphere $C$ at the position of the centroid of the segment, while the radius $r$ was defined as half the length of the bounding box longest edge. 
...
where $d_m$ is the radial distance of the center of the vesicle membrane and $t_m$ is the thickness of the vesicle membrane.
...
If the MD p-value of a specific vesicle was not in a specific margin range (0-10), their radial profile was recalculated, and the label was entirely removed if they again failed to pass the margin of the p-value.`{.yellow}
`Amin, please check what exactly was done with the outliers. 
And the p-value cannot be higher than 1, while you wrote (0-10)`{.green}
...
The framework was not only designed to evaluate quantitatively the performance of the neural network but rather to assay the segmentation of vesicles in practice `\_unsure what the last part means*`{.green}. `\_I tried to say we develop the software rather than an algorithm paper with ablation study kinda more transfer learning but however, for transfer learning, we might need to add finetuning the network or say this sentence in another way*`{.green}.
The pipeline also generates a specific output format, which is necessary to further analyze the presynaptic tomograms via another pre-developed toolbox (Pyto), which segments small molecular filaments associated with the synaptic vesicles, titled tethers and connectors.
...
`\_after! I meant from that parenthesis that radius or diameter we assume as one characteristic of vesicle we might write it better *`{.green}
The error of diameter estimation of the true-detected vesicle is defined as 1 minus the proportion of diameters 
...
The center error is an Euclidean distance of ground truth and corresponds to true predicted vesicles `\_true pos or true neg? True Predicted means true positive*`{.green}.
...
`\_we assume this as a hard condition to be true positive we could define like some % like 50% intersection but this condition is generally harder`{.green}
