...
**Evaluation Metrics for Vesicle Segmentation in Cryo-ET**

Throughout our deep learning-based segmentation process in cryo-electron tomography (cryo-ET), we consistently employ the DICE coefficient as a robust evaluation metric. 
Initially, during the training phase, we estimate the model's performance using the DICE coefficient. 
Post-training, we utilize a generalized form of the DICE coefficient, termed Soft-DICE, to evaluate the non-binary probability maps. 
This consistent use of the DICE metric, in its various forms, ensures a uniform evaluation criterion across all stages of our pipeline.

1) **Probabilistic Output Evaluation: Soft-DICE Coefficient**

In the training phase, our deep neural network produces a probability mask for each voxel, indicating its likelihood of being part of a vesicle. 
The Soft-DICE coefficient serves as an apt metric to evaluate the model's performance on these probabilistic outputs, providing a continuous measure of similarity between the predicted probability mask and the ground truth.

$$ SoftDice = 1-\frac{2 y_{true} y_{pred}}{y_{true}^2 + y_{pred}^2} $$

It's crucial to note that the Soft-DICE metric transitions to the traditional DICE coefficient upon binarizing the probability mask.

2) **Binary Segmentation Evaluation: DICE Coefficient**

Following the post-processing step, the probability mask undergoes binarization to yield the final segmented output. 
The DICE coefficient is employed to assess the model's performance on this binary segmentation, quantifying the overlap between the predicted and ground truth segmented regions.

$$ Dice = \frac{2 y_{true} y_{pred}^{'}}{y_{true}^2 + y_{pred}^{'2}} $$

Where $y_{pred}^{'}$ represents the binarized predicted labels.

This metric offers a comprehensive perspective on the model's segmentation prowess, ensuring accurate delineation of vesicles within the tomogram.
...
#### Center Error $\delta d$

The center error, denoted as $\delta d$, represents the Euclidean distance between the ground truth and the predicted vesicle centers.
It can be mathematically expressed as:

$$\delta d = \sqrt{(x_{predicted} - x_{groundtruth})^2 + (y_{predicted} - y_{groundtruth})^2 + (z_{predicted} - z_{groundtruth})^2}$$

---

#### Vesicle-wise Evaluation Metrics

Considering that the radii of the predicted and ground truth vesicles are approximately equal, the volume of intersection between the two vesicles can be defined as:

$$V = \frac{1}{12} \pi (4R + \delta d)(2R - \delta d)$$

Where $R$ represents the radius of the estimated vesicle, and $\delta d$ is the distance between their centers.

For vesicle labeling in a 3D volume, a stringent criterion is adopted.
A vesicle is classified as true-detected if the center of the predicted vesicle lies within the manually segmented vesicle and vice versa.
This criterion ensures that the minimum intersection volume between two vesicles for a true detection is approximately 30% (when $\delta d = R/2$).
Vesicles that do not meet this criterion are categorized as False Positives, and undetected vesicles are labeled as False Negatives.

---

#### Vesicle Diameter Deviation Analysis

The diameter of a vesicle is a pivotal attribute, determined based on specific criteria (refer to the Outlier Exclusion section).
To gauge the accuracy of our diameter predictions for correctly identified vesicles, we introduce a deviation metric.

Let $d_{GTi}$ represent the diameter of a manually segmented vesicle and $d_{Si}$ denote the estimated diameter of the corresponding vesicle.
The deviation in diameter estimation for each vesicle is given by:

$$\delta d = 1 - \frac{\min(d_{Si}, d_{GTi})}{\max(d_{Si}, d_{GTi})}$$

This equation provides a measure of the alignment between our estimated diameter and the manual segmentation.
A lower value of $\delta d$ indicates a more accurate diameter estimation.




#### Vesicle-wise metrics


If we imagine the radius of predicted and grandtruth is almost same we can write the intersection volume of two vesicle like below: 
...
Which R estimated vesicle, and d is distance between two center.  
For assuming a label in 3D volume we chose reletively strict crtiera.
Here a vesicle was defined as a true-detected vesicle if the predicted center was located inside the hand-segmented vesicle and the other way around the center of prediction was located inside the predicted vesicle.
...
This means the minimum intersection of two vesicle as true detected vesicle would be around 30 percent. (while d=R/2)
And otherwise we asume the predicted vesicle as False Postive and pther way round the not detected Vesicle as Flase Negative.
